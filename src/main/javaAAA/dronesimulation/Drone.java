...
public class Drone {
    ...
    public void move(){
        ...
        /*
        if (intersects(MapGenerator.s1.x, MapGenerator.e1.x, x, MapGenerator.s1.y, MapGenerator.e1.y, y)){
            SmsSender.sendToSimon();
            System.out.println("Went into a airport area.");
        }
        else if (intersects(MapGenerator.s2.x, MapGenerator.e2.x, x, MapGenerator.s2.y, MapGenerator.e2.y, y)){
            SmsSender.sendToSimon();
            System.out.println("Went into a nature area.");
        }
        */
    }
    public boolean intersects(double ax, double bx, double cx, double ay, double by, double cy){
        if (Math.abs((ax*(by-cy) + bx*(cy-ay) + cx*(ay-by))/2) <= Math.abs((bx-ax)*(by-ay))){
            return true;
        }
        return false;
    }
